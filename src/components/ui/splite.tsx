...
// Lazy load the Spline component for high-performance devices
...
export function SplineScene({ scene, className }: SplineSceneProps) {
  ...
  // Detect low-performance / mobile devices
  useEffect(() => {
    const checkPerformance = () => {
      ...
      // Screen width heuristic for mobile device check
      const isSmallScreen = window.innerWidth < 768;

      if (isMobile || isLowCores || isSmallScreen) {
  ...
  if (isLowPerformance) {
    // Return a gorgeous, high-performance static gradient/glow mockup for mobile
    return (
        ...
        <Suspense 
          fallback={
            <div className="w-full h-full flex items-center justify-center bg-transparent">
              {/* Spinner loader that matches the theme */}
